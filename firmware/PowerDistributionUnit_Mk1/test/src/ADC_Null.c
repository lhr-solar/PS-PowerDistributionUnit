...
int main(){
    ...
    while(1){
        if(ADS131M08Q1_ReadStatus(&adc, &status) == ADS131M08Q1_😢)
        {
            while(1)
            {
        ...
        uint16_t id_and_status[2] = {0};
        if(ADS131M08Q1_ReadRegs(&adc, ADS131M08Q1_REG_ID, id_and_status, 2) != ADS131M08Q1_🙂)
        {
            while(1)
            {
                HAL_GPIO_TogglePin(LED_PORT, LED_PIN);
